...
class ModbusAgc150MainsConnectorTestCase(unittest.IsolatedAsyncioTestCase):
    async def test_modbus_agc150_mains_connector(self) -> None:
        salobj.set_test_topic_subname()
        async with salobj.make_mock_write_topics(
            name="ESS",
            attr_names=["tel_agcMains150", "evt_sensorStatus"],
        ) as topics:
            config = types.SimpleNamespace(
                ...
                device_type="agc150mains",
            )
            log = logging.getLogger(type(self).__name__)
            self.modbus_agc150_connector = ModbusAgc150MainsConnector(
                config=config,
                topics=topics,
                log=log,
                simulation_mode=1,
    ...
    async def test_process_telemetry(self) -> None:
        ...
        async with salobj.make_mock_write_topics(
            ...
            self.assertTrue(hasattr(topics, "tel_agcMains150"))
            self.assertTrue(topics.tel_agcMains150.has_data)
    ...
    async def test_read_coils(self) -> None:
        ...
        async with salobj.make_mock_write_topics(
            ...
            log = logging.getLogger(type(self).__name__)
            self.modbus_agc150_connector = ModbusAgc150MainsConnector(config=config, topics=topics, log=log)
